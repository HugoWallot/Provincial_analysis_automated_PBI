...
-   Gross domestic product (GDP) at basic prices, by industry, provinces and territories, contributions to national percentage change [36-10-0658-01](https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=3610065801)
...
gdp_CPC_national_provinces_per_industry_table_number <- "36-10-0658-01"
#step 1.3
...
# only the first cansim table can be loaded se we will use the monthly table
...
```
# gdp CPC national provinces per industry Step 1.2 

```{r database for gdp CPC national provinces per industry,  echo=FALSE, include=FALSE}
#Loading the data base - using cansim function to obtain the ndm format of the table.

gdp_CPC_national_provinces_per_industry <- cansim::get_cansim(gdp_CPC_national_provinces_per_industry_table_number)
gdp_CPC_national_provinces_per_industry <- janitor::clean_names(gdp_CPC_national_provinces_per_industry) # get rid of capital letters in col names

```

```{r gdp CPC national provinces per industry formatting}

unique(gdp_CPC_national_provinces_per_industry$value_2)

gdp_CPC_national_provinces_per_industry %>%
  select(ref_date, geo, north_american_industry_classification_system_naics, value, classification_code_for_north_american_industry_classification_system_naics) %>%
  filter(ref_date > from_this_year) %>%
  filter(classification_code_for_north_american_industry_classification_system_naics %in% c(two_digit_naics_code)) %>%
   distinct() %>%
  select(!classification_code_for_north_american_industry_classification_system_naics) %>%
  #select(! value_2) %>%
  rename_if(is.numeric, funs(str_c("value"))) ->
clean_gdp_CPC_national_provinces_per_industry


write.csv(clean_gdp_CPC_national_provinces_per_industry, file = "clean_gdp_CPC_national_provinces_per_industry_step1.2.csv")

interactive_table(clean_gdp_CPC_national_provinces_per_industry)
...
Transit_gdp_canada_per_industry %>%
  select(ref_year, geo, north_american_industry_classification_system_naics, value_fd, value_pc) %>%
  rename(total_GDP_growth_canada = value_fd) %>%
  rename(canada_percent_change_per_industry = value_pc) %>%
...
Transit_gdp_provinces_per_industry %>%
  select(ref_date, geo, north_american_industry_classification_system_naics, value_pc, value_2, value_fd) %>%
...
Transit_gdp_canada_per_industry %>%
  select(ref_year, geo, north_american_industry_classification_system_naics, value_pc, value_fd) %>%
  rename(ref_date = ref_year)  ->
...
provinces_gdp_CPC_canada_gdp %>%
  mutate(gdp_contribution_to_national_percent_change = value_fd/total_GDP_growth_canada) %>%
  mutate(percent_gdp_contribution_to_national_percent_change = gdp_contribution_to_national_percent_change*canada_percent_change_per_industry) %>%
